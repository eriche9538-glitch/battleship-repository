...
// .wrangler/tmp/bundle-L3lkuW/checked-fetch.js
...
async function onRequestGet(context) {
  ...
  try {
    ...
    const mode = url.searchParams.get("mode") === "currency" ? "currency" : "wins";
    const rankingColumn = mode === "currency" ? "battle_currency" : "score";
    const db = env.DB;
    if (!db) {
      return jsonResponse4(500, { error: "Database binding 'DB' is missing." });
    }
    const topUsers = await db.prepare(`SELECT id, username, COALESCE(${rankingColumn}, 0) AS value FROM Users ORDER BY COALESCE(${rankingColumn}, 0) DESC, lower(username) ASC LIMIT 100`).all();
    ...
    let yourEntry = null;
    if (userId) {
      const currentUser = await db.prepare(`SELECT id, username, COALESCE(${rankingColumn}, 0) AS value FROM Users WHERE id = ?1 LIMIT 1`).bind(userId).first();
      if (currentUser) {
        const currentScore = currentUser.value ?? 0;
        const userRank = await db.prepare(`SELECT COUNT(*) + 1 AS rank FROM Users WHERE COALESCE(${rankingColumn}, 0) > ?1 OR (COALESCE(${rankingColumn}, 0) = ?1 AND lower(username) < lower(?2))`).bind(currentScore, currentUser.username).first();
...
// .wrangler/tmp/bundle-L3lkuW/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-L3lkuW/middleware-loader.entry.ts
