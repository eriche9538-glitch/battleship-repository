...
// .wrangler/tmp/bundle-rPrcxE/checked-fetch.js
...
async function onRequestPost3(context) {
  ...
  try {
    ...
    if (!Number.isFinite(parsedEloDelta) || !["normal", "blitz"].includes(eloMode)) {
      return jsonResponse3(400, { error: "Invalid ELO update" });
    }
    const eloColumn = eloMode === "blitz" ? "blitz_elo" : "normal_elo";
    await db.prepare(`UPDATE Users SET score = score + 1, ${eloColumn} = ${eloColumn} + ?2, battle_currency = MAX(0, COALESCE(battle_currency, 0) + ?3) WHERE id = ?1`).bind(userId, parsedEloDelta, parsedCurrencyDelta).run();
...
async function onRequestGet(context) {
  try {
    ...
    const user = await context.env.DB.prepare("SELECT id, username, email, score, COALESCE(battle_currency, 0) AS battle_currency, COALESCE(normal_elo, 0) AS normal_elo, COALESCE(blitz_elo, 0) AS blitz_elo FROM Users WHERE id = ?1 LIMIT 1").bind(userId).first();
    if (!user) {
...
async function onRequestGet2(context) {
  ...
  try {
    ...
    const topUsers = currencyMode ? await db.prepare("SELECT id, username, COALESCE(battle_currency, 0) AS currency FROM Users ORDER BY COALESCE(battle_currency, 0) DESC, lower(username) ASC LIMIT 100").all() : eloMode === "blitz-elo" ? await db.prepare("SELECT id, username, COALESCE(blitz_elo, 0) AS elo FROM Users ORDER BY COALESCE(blitz_elo, 0) DESC, lower(username) ASC LIMIT 100").all() : eloMode === "normal-elo" ? await db.prepare("SELECT id, username, COALESCE(normal_elo, 0) AS elo FROM Users ORDER BY COALESCE(normal_elo, 0) DESC, lower(username) ASC LIMIT 100").all() : await db.prepare("SELECT id, username, score FROM Users ORDER BY COALESCE(score, 0) DESC, lower(username) ASC LIMIT 100").all();
    ...
    let yourEntry = null;
    if (userId) {
      const currentUser = currencyMode ? await db.prepare("SELECT id, username, COALESCE(battle_currency, 0) AS currency FROM Users WHERE id = ?1 LIMIT 1").bind(userId).first() : eloMode === "blitz-elo" ? await db.prepare("SELECT id, username, COALESCE(blitz_elo, 0) AS elo FROM Users WHERE id = ?1 LIMIT 1").bind(userId).first() : eloMode === "normal-elo" ? await db.prepare("SELECT id, username, COALESCE(normal_elo, 0) AS elo FROM Users WHERE id = ?1 LIMIT 1").bind(userId).first() : await db.prepare("SELECT id, username, score FROM Users WHERE id = ?1 LIMIT 1").bind(userId).first();
      if (currentUser) {
        const currentScore = currencyMode ? currentUser.currency ?? 0 : eloMode ? currentUser.elo ?? 0 : currentUser.score ?? 0;
        const userRank = await db.prepare(currencyMode ? "SELECT COUNT(*) + 1 AS rank FROM Users WHERE COALESCE(battle_currency, 0) > ?1 OR (COALESCE(battle_currency, 0) = ?1 AND lower(username) < lower(?2))" : eloMode === "blitz-elo" ? "SELECT COUNT(*) + 1 AS rank FROM Users WHERE COALESCE(blitz_elo, 0) > ?1 OR (COALESCE(blitz_elo, 0) = ?1 AND lower(username) < lower(?2))" : eloMode === "normal-elo" ? "SELECT COUNT(*) + 1 AS rank FROM Users WHERE COALESCE(normal_elo, 0) > ?1 OR (COALESCE(normal_elo, 0) = ?1 AND lower(username) < lower(?2))" : "SELECT COUNT(*) + 1 AS rank FROM Users WHERE COALESCE(score, 0) > ?1 OR (COALESCE(score, 0) = ?1 AND lower(username) < lower(?2))").bind(currentScore, currentUser.username).first();
...
// .wrangler/tmp/bundle-rPrcxE/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-rPrcxE/middleware-loader.entry.ts
